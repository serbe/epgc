...
func (e *Edb) GetContact(id int64) (Contact, error) {
	...
	FROM
		contacts AS p
	LEFT JOIN emails AS e ON p.id = e.contact_id
	LEFT JOIN phones AS ph ON p.id = ph.contact_id AND ph.fax = false
	LEFT JOIN phones AS f ON p.id = f.contact_id AND f.fax = true
	WHERE p.id = $1
	GROUP BY p.id`)
	if err != nil {
...
func (e *Edb) CreateContact(contact Contact) (int64, error) {
	stmt, err := e.db.Prepare(`INSERT INTO contacts(name, company_id, post_id, post_go_id, rank_id, birthday, note, created_at) VALUES($1, $2, $3, $4, $5, $6, $7, now()) RETURNING id`)
	if err != nil {
		log.Println("CreateContact e.db.Prepare ", err)
		return 0, err
	}
	err = stmt.QueryRow(s2n(contact.Name), i2n(contact.CompanyID), i2n(contact.PostID), i2n(contact.PostGOID), i2n(contact.RankID), sd2n(contact.Birthday), s2n(contact.Note)).Scan(&contact.ID)
	if err != nil {
...
func (e *Edb) UpdateContact(contact Contact) error {
	stmt, err := e.db.Prepare(`UPDATE contacts SET name=$2, company_id=$3, post_id=$4, post_go_id=$5, rank_id=$6, birthday=$7, note=$8, updated_at = now() WHERE id = $1`)
	if err != nil {
		log.Println("UpdateContact e.db.Prepare ", err)
		return err
	}
	_, err = stmt.Exec(i2n(contact.ID), s2n(contact.Name), i2n(contact.CompanyID), i2n(contact.PostID), i2n(contact.PostGOID), i2n(contact.RankID), sd2n(contact.Birthday), s2n(contact.Note))
	if err != nil {
...
func (e *Edb) contactCreateTable() error {
	str := `CREATE TABLE IF NOT EXISTS contacts (id bigserial primary key, name text, company_id bigint, post_id bigint, post_go_id bigint, rank_id bigint, birthday date, note text, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone)`
	_, err := e.db.Exec(str)
	if err != nil {
